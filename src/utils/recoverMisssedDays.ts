...
export const recoverMissedDays = async (hrModels: HRModels) => {
    try {
        ...
        const lastRunDateValue =
            cronLog?.getDataValue("lastRunDate") as string | undefined;
        ...
        console.log(
            `⏳ Recovering attendance from ${startDate.format("YYYY-MM-DD")} to ${today
                .clone()
                .subtract(1, "day")
                .format("YYYY-MM-DD")}`
        );
        ...
        console.error("❌ Failed to recover attendance:", error);
    }
};
